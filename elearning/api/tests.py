...
class TestAPI(TestCase):
    def setUp(self):
        ...
        self.test_course_materials = CourseMaterial.objects.create(
            course=self.course, uploaded_by=self.teacher_app_user, title="Test Material"
        )
        # Create default status updates
        self.status_update = UserStatusUpdate.objects.create(
            user=self.student_user,
            status_title="Test Status",
            status_content="Test Status Content",
        )
        self.status_update = UserStatusUpdate.objects.create(
        ...
        self.notification = Notification.objects.create(
            user=self.student_app_user, message="Test Notification Message"
        )
    ...
    def test_user_enrolled_courses_api(self):
        ...
        self.assertEqual(
            response.data[0]["course_description"], "Test Course Description"
        )
        self.assertEqual(
            response.data[0]["created_by"]["user"]["username"], "teacher_1"
        )
