...
class UserEnrolledCoursesView(generics.ListAPIView):
  ...
  def get_queryset(self):
    ...
    return Course.objects.filter(enrolled_students=app_user).order_by('-time_created')
...
class CourseFeedbackView(generics.ListAPIView):
  ...
  def get_queryset(self):
    ...
    if app_user.is_student:
      return CourseFeedback.objects.filter(course=course).order_by('-time_submitted')
...
class CourseMaterialView(generics.ListAPIView):
  ...
  def get_queryset(self):
    ...
    if app_user.is_student:
      return CourseMaterial.objects.filter(course__id=course_id, course__enrolled_students=app_user).order_by('-upload_time')
    elif app_user.is_teacher:
      return CourseMaterial.objects.filter(course__id=course_id, course__created_by=app_user).order_by('-upload_time')