...
class TestCourses(TestCase):
    def setUp(self):
        ...
        self.student_user = User.objects.create_user(
            username="student_1", password="abcd"
        )
        self.student_app_user = AppUser.objects.create(
            user=self.student_user, is_student=True
        )
        # Create a teacher user
        self.teacher_user = User.objects.create_user(
            username="teacher_1", password="abcd"
        )
        self.teacher_app_user = AppUser.objects.create(
            user=self.teacher_user, is_teacher=True
        )
        # Create a course and enroll the student
        self.course = Course.objects.create(
            course_title="Test Course",
            course_description="Test Course Description",
            created_by=self.teacher_app_user,
        )
        self.course.enrolled_students.add(self.student_app_user)
        self.course_feedback = CourseFeedback.objects.create(
            course=self.course,
    ...
    def test_create_course_function(self):
        ...
        response = self.client.post(
            "/user/course/create/",
            {
                "course_title": "Teacher 1 Course",
                "course_description": "Teacher 1 Course Description",
            },
        )
    ...
    def test_submit_feedback_function(self):
        ...
        response = self.client.post(
            "/user/course/1/feedback/submit/", {"feedback_content": "Test Feedback 2"}
        )
        # Check if the page redirects correctly
        self.assertEqual(response.status_code, 302)
        ...
        self.assertEqual(
            CourseFeedback.objects.last().feedback_content, "Test Feedback 2"
        )
    ...
    def test_upload_course_materials_function(self):
        ...
        test_file = SimpleUploadedFile(
            "test_file.pdf", b"test_file_content", content_type="application/pdf"
        )
        # Upload the test file as a course material
        response = self.client.post(
            "/user/course/1/upload/", {"title": "Test Material", "file": test_file}
        )
        # Check if the page redirects correctly
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "/user/course/1/view/")
    ...
    def test_enrollment_notification_function(self):
        ...
        self.assertEqual(
            Notification.objects.last().message,
            "student_1 has enrolled in your course Test Course",
        )
    ...
    def test_upload_notification_function(self):
        ...
        self.assertEqual(
            Notification.objects.last().message,
            "New course material uploaded for Test Course by teacher_1",
        )
