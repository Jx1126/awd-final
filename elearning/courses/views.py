...
def enroll_course(request, course_id):
    ...
    if app_user in course.enrolled_students.all():
        ...
        Notification.objects.create(
            ...
            message=f"{app_user.user.username} has enrolled in your course {course.course_title}",
        )
...
def load_feedbacks(request, course_id):
    ...
    return render(
        request,
        "courses/course_feedback.html",
        {"course": course, "feedbacks": feedbacks, "feedback_form": feedback_form},
    )
...
def upload_course_materials(request, course_id):
    ...
    if app_user != course.created_by:
        messages.error(
            request, "You do not have permission to upload course materials."
        )
        return HttpResponseRedirect(f"/user/course/{course.id}/view/")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for student in enrolled_students:
                Notification.objects.create(
                    user=student,
                    message=f"New course material uploaded for {course.course_title} by {app_user.user.username}",
                )

            # Process the course materials asynchronously
            process_course_materials.delay(
                course_id, app_user.id, course_material.title, course_material.file.path
            )
